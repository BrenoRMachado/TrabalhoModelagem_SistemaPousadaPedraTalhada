    ...
    <form action="/admin/reservas/criar" method="POST" class="modal-nova-reserva">
        ...
        <div class="nova-reserva-input-container">
            <div class="nova-reserva-input">
                ...
                <input type="date" name="dataEntradaPrevista" id="dataEntrada" required>
            </div>
            <div class="nova-reserva-input">
                <label>Data de Saída <span class="required">*</span></label>
                <input type="date" name="dataSaidaPrevista" id="dataSaida" required>
            </div>
        </div>
        ...
        <div class="nova-reserva-input-quarto">
            ...
            <select name="idQuarto" id="quarto-select" required>
                ...
                <?php if(isset($quartos) && !empty($quartos)): ?>
                    <?php foreach($quartos as $quarto): ?>
                        <option value="<?= $quarto->numero ?>" data-preco="<?= $quarto->precoDiaria ?>">
                            Quarto <?= $quarto->numero ?> - <?= $quarto->tipo ?> (R$ <?= number_format($quarto->precoDiaria, 2, ',', '.') ?>)
                        </option>
                    <?php endforeach; ?>
                <?php else: ?>
                    <option value="" disabled>Nenhum quarto disponível no momento</option>
                <?php endif; ?>
            </select>
        </div>
        ...
        <div class="nova-reserva-input-container">
            ...
            <div class="nova-reserva-input">
                ...
                <input type="text" name="cpf" id="cpf_hospede" placeholder="Ex: 000.000.000-00" required>
            </div>
        </div>
        ...
        <div class="nova-reserva-input-quarto">
            ...
            <textarea name="observacoes" placeholder="Notas especiais, requisições, alergias, etc..."></textarea>
        </div>
